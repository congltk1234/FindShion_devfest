...
def img_object_detection_outfit(file: bytes = File(...)):
    ...
    for i in range(outfit.shape[0]):
        ...
        print(label)
        for i in f_ids:
            id_articles.append(str(articles['article_id'][i]))
            print(articles['detail_desc'][i])
        print(id_articles)

        item1 = dict({"name": label, "id": id_articles})
        item.append(item1)
    ...
    item_dict = dict({"item": item})

    # result=json.dumps(item_dict)
